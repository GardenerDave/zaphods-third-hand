#!/usr/bin/env python3
"""Freshness, difficulty, and manifest helpers for frozen Run 7 scope fixtures."""
...
DIFFICULTY_PATH = "docs/research/RUN_7_SCOPE_DIFFICULTY_CRITERIA_FREEZE_2026-08-20.json"
PROVENANCE_PATH = "docs/research/RUN_7_FIXTURE_PROVENANCE_2026-08-20.md"


class Run7FixtureError(ValueError):
    pass
...
def _difficulty_criteria(repo_root: Path) -> dict[str, Any]:
    return json.loads((repo_root / DIFFICULTY_PATH).read_text(encoding="utf-8"))
...
def novelty_audit(pack_dir: Path, repo_root: Path) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "schema": "zth_run7_scope_novelty_audit_v1",
    ...
    result["counts"] = {
        ...
        "within_pack_source_document_reuse": max(0, len(new_tasks) - len({t["provenance"]["source_document"] for t in new_tasks})),
        "within_pack_source_anchor_reuse": len(new_tasks) - len({(t["provenance"]["source_document"], t["provenance"]["source_anchor"]) for t in new_tasks}),
    }
    return result
...
def build_manifest(pack_dir: Path, repo_root: Path, *, generated_at: str) -> tuple[dict[str, Any], dict[str, Any]]:
    if pack_dir != repo_root / PACK_ROOT / PACK_DIRECTORY:
        raise Run7FixtureError(f"unknown Run 7 pack: {pack_dir}")
    paths = fixture_paths(pack_dir)
    if len(paths) != CANDIDATE_COUNT:
        raise Run7FixtureError(f"expected {CANDIDATE_COUNT} candidates, found {len(paths)}")
    criteria = _difficulty_criteria(repo_root)
    tasks = [load_task_fixture(p) for p in paths]
    if any(t["task_family"] != FAMILY for t in tasks): raise Run7FixtureError("family drift")
    if len({t["task_id"] for t in tasks}) != CANDIDATE_COUNT: raise Run7FixtureError("task IDs are not unique")
    allowed = set(criteria["allowed_structural_features"])
    for task in tasks:
        result = _validator_result(json.dumps(representative_output(task), sort_keys=True), task, attempt_id=f"run7-satisfiability-{task['task_id']}")
        if result["validation_status"] != "passed": raise Run7FixtureError(f"unsatisfiable fixture: {task['task_id']}")
        calibration = task.get("calibration", {})
        features = set(calibration.get("difficulty_features", []))
        if calibration.get("target_evidence_resolution") != RESOLUTION or calibration.get("target_evidence_key") != FAMILY: raise Run7FixtureError(f"target binding mismatch: {task['task_id']}")
        if calibration.get("difficulty_band") != "enriched" or calibration.get("selection_basis") != "structural_only" or calibration.get("model_outputs_consulted") is not False: raise Run7FixtureError(f"difficulty binding mismatch: {task['task_id']}")
        if len(features) < criteria["minimum_structural_features_per_candidate"] or not features <= allowed: raise Run7FixtureError(f"difficulty features mismatch: {task['task_id']}")
        failed = _validator_result(json.dumps({"allowed_targets": [], "held_targets": [], "scope_expansion_required": False, "review_status": "ready_for_review"}, sort_keys=True), task, attempt_id=f"run7-signature-{task['task_id']}")
        if calibration.get("target_failure_classes") != failure_classes(failure_signature(FAMILY, failed)): raise Run7FixtureError(f"failure-class binding mismatch: {task['task_id']}")
    audit = novelty_audit(pack_dir, repo_root)
    if any(audit[k] for k in ("task_id_collisions", "exact_prompt_duplicates", "normalized_prompt_duplicates", "high_similarity_pairs")): raise Run7FixtureError("freshness audit found a collision or high-similarity pair")
    records = []
    for path, task in zip(paths, tasks):
        records.append({"path": str(path.relative_to(repo_root)), "task_id": task["task_id"], "task_family": task["task_family"], "novelty": task["provenance"]["novelty"], "source_document": task["provenance"]["source_document"], "source_anchor": task["provenance"]["source_anchor"], "fixture_sha256": sha256_bytes(path.read_bytes()), "output_contract_sha256": sha256_json(task["output_contract"]), "reference_facts_sha256": sha256_json(task["validator"].get("reference_facts", {})), "target_evidence_resolution": task["calibration"]["target_evidence_resolution"], "target_evidence_key": task["calibration"]["target_evidence_key"], "difficulty_features": task["calibration"]["difficulty_features"]})
    audit["audit_sha256"] = sha256_bytes(canonical(audit).encode())
    task_ids = [t["task_id"] for t in tasks]
    manifest = {"schema": "zth_run7_scope_fixture_manifest_v1", "pack_id": pack_dir.name, "generated_at": generated_at, "model_outputs_consulted": False, "candidate_count": CANDIDATE_COUNT, "target_included_count": TARGET_COUNT, "task_family": FAMILY, "target_evidence_resolution": RESOLUTION, "difficulty_criteria_path": DIFFICULTY_PATH, "difficulty_criteria_sha256": sha256_bytes((repo_root / DIFFICULTY_PATH).read_bytes()), "fixtures": records, "candidate_order": task_ids, "eligibility": ["transport_valid=true", "transport_classification=model_response", "deterministic baseline validation failure", "target resolution=failure_class", "target evidence key=scope-authority-boundary", "frozen structural difficulty criteria satisfied"], "selection_rule": "Select the first target count eligible failures in frozen candidate order; never use intervention outcomes for inclusion.", "reserve_rule": "Candidates not selected after baseline eligibility remain reserve-only; no adaptive replacement.", "pair_order": {"seed": PAIR_ORDER_SEED, "algorithm": "sha256(str(seed) + ':' + task_id), first hex digit < 8 means control-first", "orders": pair_orders(task_ids)}, "novelty_audit_path": str((pack_dir / "novelty_audit.json").relative_to(repo_root)), "novelty_audit_sha256": audit["audit_sha256"], "provenance_path": PROVENANCE_PATH, "provenance_sha256": sha256_bytes((repo_root / PROVENANCE_PATH).read_bytes()), "manifest_sha256": None, "pack_sha256": None}
...
def verify_manifest(pack_dir: Path, repo_root: Path) -> dict[str, Any]:
    ...
    if manifest != expected: raise Run7FixtureError(f"manifest drift: {pack_dir}")
    return manifest
