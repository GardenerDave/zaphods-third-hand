#!/usr/bin/env python3
"""Self-verifying fresh fixture pack for the targeted Run 4B scope replication."""
...
PACK_ID = "reviewed_run4b_scope"
TASK_FAMILY = "scope-authority-boundary"
TARGET_RESOLUTION = "failure_class"
TARGET_COUNT = 12
CANDIDATE_COUNT = 15
PAIR_ORDER_SEED = 20260823
SIMILARITY_THRESHOLD = 0.80
PRIOR_PACKS = (
    "reviewed_v1", "reviewed_v2", "reviewed_v3", "reviewed_v3b", "reviewed_v3c",
    "reviewed_v4a", "reviewed_run4_economic_triage",
)


class Run4BScopeFixtureError(ValueError):
    pass
...
def prior_tasks(repo_root: Path) -> list[dict[str, Any]]:
    ...
    for pack in PRIOR_PACKS:
        directory = repo_root / "local_harness/fixtures/capability_loop" / pack
        if not directory.exists():
            continue
        for path in sorted(directory.glob("*.json")):
            if path.name not in {"manifest.json", "novelty_audit.json"}:
...
def representative_output(task: dict[str, Any]) -> dict[str, Any]:
    ...
    output: dict[str, Any] = {}
    for field in task["output_contract"]["required_fields"]:
        if field == "allowed_targets":
            ...
            output[field] = "bounded review evidence"
    return output
...
def novelty_audit(pack_dir: Path, repo_root: Path) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "schema": "zth_run4b_scope_novelty_audit_v1",
...
def select_included_candidates(candidate_order: list[str], eligible_ids: set[str], target_count: int = TARGET_COUNT) -> tuple[list[str], list[str]]:
    selected = [task_id for task_id in candidate_order if task_id in eligible_ids][:target_count]
    return selected, [task_id for task_id in candidate_order if task_id not in selected]
...
def build_manifest(pack_dir: Path, repo_root: Path, *, generated_at: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if len(paths) != CANDIDATE_COUNT:
        raise Run4BScopeFixtureError(f"expected {CANDIDATE_COUNT} candidates, found {len(paths)}")
    tasks = [load_task_fixture(path) for path in paths]
    if len({task["task_id"] for task in tasks}) != CANDIDATE_COUNT or any(task["task_family"] != TASK_FAMILY for task in tasks):
        raise Run4BScopeFixtureError("candidate identity/family mismatch")
    for task in tasks:
        for key in task["validator"].get("reference_facts", {}):
            spec = REFERENCE_FACT_SPECS.get(key)
            if spec is None or spec.source_metadata:
                raise Run4BScopeFixtureError(f"unsupported reference fact: {key}")
        witness = json.dumps(representative_output(task), sort_keys=True)
        result = _validator_result(witness, task, attempt_id=f"run4b-satisfiability-{task['task_id']}")
        if result["validation_status"] != "passed":
            raise Run4BScopeFixtureError(f"unsatisfiable fixture: {task['task_id']}")
        signature = failure_signature(task["task_family"], _validator_result(json.dumps({"allowed_targets": [], "held_targets": [], "scope_expansion_required": True, "review_status": "ready_for_review"}), task, attempt_id=f"run4b-signature-{task['task_id']}"))
        if task["calibration"].get("target_evidence_resolution") != TARGET_RESOLUTION or task["calibration"].get("target_evidence_key") != TASK_FAMILY:
            raise Run4BScopeFixtureError(f"target resolution mismatch: {task['task_id']}")
        if task["calibration"].get("target_failure_classes") != failure_classes(signature):
            raise Run4BScopeFixtureError(f"failure-class binding mismatch: {task['task_id']}")
    ...
    if any(audit[key] for key in ("task_id_collisions", "exact_prompt_duplicates", "normalized_prompt_duplicates", "high_similarity_pairs")):
        raise Run4BScopeFixtureError("freshness audit found a collision or high-similarity pair")
    records = []
    for path, task in zip(paths, tasks):
        records.append({
            ...
            "target_failure_classes": task["calibration"]["target_failure_classes"],
        })
    candidate_order = [task["task_id"] for task in tasks]
    audit_digest = sha256_bytes(canonical(audit).encode())
    manifest = {
        "schema": "zth_run4b_scope_fixture_manifest_v1", "pack_id": PACK_ID, "generated_at": generated_at,
        "model_outputs_consulted": False, "candidate_count": CANDIDATE_COUNT, "target_included_count": TARGET_COUNT,
        "task_family": TASK_FAMILY, "target_evidence_resolution": TARGET_RESOLUTION, "target_evidence_key": TASK_FAMILY,
        "fixtures": records, "candidate_order": candidate_order,
        "eligibility": ["transport_valid=true", "transport_classification=model_response", "deterministic baseline validation failure", "target resolution=failure_class", "target evidence key=scope-authority-boundary"],
        "selection_rule": "Select the first 12 eligible candidates in frozen order; intervention outcomes never affect inclusion.",
        "reserve_rule": "Candidates not selected remain reserve-only; no adaptive replacement.",
        "pair_order": {"seed": PAIR_ORDER_SEED, "algorithm": "sha256(str(seed) + ':' + task_id), first hex digit < 8 means control-first", "orders": pair_orders(candidate_order)},
        "interventions": {"control": "external_teacher", "treatment": "local_teacher", "no_deterministic_patch": True, "no_fallback_escalation": True},
        "novelty_audit_path": str((pack_dir / "novelty_audit.json").relative_to(repo_root)),
        "novelty_audit_sha256": audit_digest, "manifest_sha256": None, "pack_sha256": None,
    }
    pack_basis = dict(manifest); pack_basis["manifest_sha256"] = None; pack_basis["pack_sha256"] = None
...
def verify_manifest(pack_dir: Path, repo_root: Path) -> dict[str, Any]:
    ...
    if manifest != expected:
        raise Run4BScopeFixtureError("Run 4B fixture manifest drift")
    return manifest
