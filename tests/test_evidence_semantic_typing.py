...
def test_task_b_bundle_derives_raw_response_integrity_only():
    ...
    assert derived["raw_response_integrity"].trust_class == "system_derived"
...
def test_handoff_bundle_derives_bounded_handoff_and_semantic_acceptance():
    ...
    derived = {item.property: item for item in result.derived_properties}
    assert derived["bounded_handoff_success"].trust_class == "system_derived"
    assert derived["semantic_acceptance"].trust_class == "system_derived"
    assert "semantic_capability" in result.unknown_properties
