...
def operation_derivation(request: str, *, model_evidence: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if model_evidence is not None:
        ...
        base.update(status=result["status"], derivation_type="SEMANTIC_INTERPRETATION", canonical_operation=result["canonical_operation"], rule_id=result["rule_id"], reason=result["reason"], model_required=False, source_fields=["input_request","prior_model.action"] , model_expression=expression, normalizer=result)
        return base
...
def closeout(out: Path) -> None:
    ...
    agg={"schema":"zth_deterministic_first_semantic_fallback_aggregate_v0","tasks_correct":sum(r["task_correct"] for r in rows),"task_count":len(rows),"deterministic_operation_derivations":sum(r["deterministic_derivation_status"]=="RESOLVED" for r in rows),"deterministic_sufficient_opportunities":len(deterministic_rows),"semantic_fallbacks_planned":sum(r["expected_model_required"] for r in rows),"semantic_fallbacks_successful":sum(r["regime"]=="SEMANTIC_FALLBACK_REQUIRED" and r["runtime_terminal_state"]=="terminal_success" for r in rows),"model_calls":sum(r["model_calls"] for r in rows),"model_calls_avoided_by_deterministic_derivation":sum(r["plan_0_model_free"] for r in deterministic_rows),"deterministic_target_derivations":sum(r["target_derivation_possible"] for r in rows),"canonical_operation_correct_applicable":sum(r["task_correct"] for r in rows if r["expected_model_required"] or r["regime"]=="DETERMINISTIC_SUFFICIENT"),"authorized_tool_calls":sum(r["tool_calls"] for r in rows),"valid_observations":sum(r["observation_status"]=="VALID_OBSERVATION" for r in rows),"terminal_success":sum(r["runtime_terminal_state"]=="terminal_success" for r in rows),"ready_for_review":sum(r["runtime_terminal_state"]=="ready_for_review" for r in rows),"runtime_expected_field_reads":0,"planner_hint_input_fields":0,"MODEL_OUTPUT_GRANTED_AUTHORITY":0,"duplicate_calls":0,"retries":0,"teacher_calls":0,"30b_calls":0,"external_calls":0,"qualification_change":False,"model_latency_ms":{"mean":statistics.mean(lat) if lat else None,"median":statistics.median(lat) if lat else None,"p95":sorted(lat)[min(len(lat)-1,round((len(lat)-1)*.95))] if lat else None},"model_gross_gpu_energy_joules":{"total":sum(energy),"mean":statistics.mean(energy) if energy else None,"median":statistics.median(energy) if energy else None},"rows":rows,"DETERMINISTIC_FIRST_CAPABILITY_ROUTING_DEMONSTRATED":all(r["task_correct"] for r in deterministic_rows),"SEMANTIC_MODEL_FALLBACK_DEMONSTRATED":all(r["task_correct"] for r in fallback_rows),"MODEL_CALL_AVOIDANCE_FROM_CAPABILITY_DECOMPOSITION_DEMONSTRATED":sum(r["plan_0_model_free"] for r in deterministic_rows)==len(deterministic_rows),"DYNAMIC_INTELLIGENCE_SURFACE_MINIMIZATION_DEMONSTRATED":all(r["task_correct"] for r in deterministic_rows+fallback_rows),"next_decision":"GENERALIZE_DETERMINISTIC_FIRST_ROUTING_TO_ADDITIONAL_CAPABILITIES" if all(r["task_correct"] for r in rows) else "DIAGNOSE_SEMANTIC_FALLBACK_INTERFACE"}
    write_json(out/"aggregate.json",agg); write_json(out/"matrix.json",{"schema":"zth_deterministic_first_semantic_fallback_matrix_v0","rows":rows,"aggregate":agg,"execution_driver_sha256":read_json(out/"router_manifest.json")["driver_sha256"] if "driver_sha256" in read_json(out/"router_manifest.json") else None,"closeout_driver_sha256":sha_file(Path(__file__))}); write_json(out/"lifecycle.json",{"status":"closeout_complete",**{k:agg[k] for k in ("model_calls","authorized_tool_calls","teacher_calls","retries","external_calls","runtime_expected_field_reads","planner_hint_input_fields")}}); print(json.dumps(agg,indent=2,sort_keys=True))
