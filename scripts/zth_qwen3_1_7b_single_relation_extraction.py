...
def execute(output_dir:Path)->None:
    ...
    rows=[]
    for task in binding["tasks"]:
        d=output_dir/"tasks"/task["task_id"]; sampler=PowerSampler(lambda:read_gpu_power(EXPECTED_GPU_UUID,base_url=telemetry_url),expected_gpu_uuid=EXPECTED_GPU_UUID,sample_interval_seconds=POWER_INTERVAL_SECONDS); started=time.monotonic(); started_at=now(); sampler.start(); response=decomposition.structured_call(spec,(d/"prompt.txt").read_text(),binding["format"],MAX_OUTPUT_TOKENS,TIMEOUT_SECONDS); captured=time.monotonic(); samples=sampler.stop(); energy=integrate_energy_joules(samples,sample_interval_seconds=POWER_INTERVAL_SECONDS,expected_gpu_uuid=EXPECTED_GPU_UUID); metadata=response.metadata(); metadata["request_url"]=None; raw={"status":response.status,"content":response.content,"metadata":metadata,"request_started_at":started_at,"response_captured_at":now(),"wall_elapsed_ms":round((captured-started)*1000,3)}; write_json(d/"response.json",raw); transport=metadata.get("transport_classification")=="model_response"; parsed,parse_valid,contract_valid,diagnostics=parse_raw(response.content) if transport else (None,False,False,["transport failure"]); scored=score(parsed,parse_valid,contract_valid,task); write_json(d/"validation.json",{"validation_status":"passed" if transport and contract_valid else "failed","parse_valid":parse_valid,"contract_valid":contract_valid,"diagnostics":diagnostics,"task_id":task["task_id"]}); write_json(d/"power_samples.json",{"measurement_level":2,"measurement_boundary":"gpu_device_only","gpu_uuid":EXPECTED_GPU_UUID,"samples":[{"timestamp_utc":s.timestamp_utc,"monotonic_seconds":s.monotonic_seconds,"gpu_uuid":s.gpu_uuid,"power_watts":s.power_watts,"sequence":s.sequence} for s in samples]}); card={"schema":"zth_qwen3_1_7b_single_relation_task_scorecard_v1","task_id":task["task_id"],"semantic_regime":task["semantic_regime"],"expected_fields":{field:task[f"expected_{field}"] for field in FIELDS},"transport_valid":transport,"raw_parse_valid":parse_valid,"contract_valid":contract_valid,"observed_fields":parsed,**scored,"wall_elapsed_ms":raw["wall_elapsed_ms"],"prompt_tokens":(metadata.get("usage") or {}).get("prompt_tokens"),"completion_tokens":(metadata.get("usage") or {}).get("completion_tokens"),"power_summary":{"sample_count":len(samples),"mean_active_watts":statistics.mean(s.power_watts for s in samples),"peak_observed_watts":max(s.power_watts for s in samples),"gross_energy_joules":energy},"retry_count":0,"escalation_count":0,"teacher_calls":0}; write_json(d/"scorecard.json",card); rows.append(card)
