...
from local_harness.atomic_capability_press import (
    ...
    component_vector,
    score_scope_object,
    target_metrics as generic_target_metrics,
)
...
# Keep this historical adapter path import-compatible while routing scoring
# through the reusable hardened implementation. Existing matrices remain
# frozen artifacts and are not regenerated by this assignment.
target_metrics = generic_target_metrics
score_object = score_scope_object
...
def aggregate(run: dict[str, Any], *, normalized: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "no_overlap_observed_and_correct": sum(score["authority_separation"]["no_allowed_held_overlap"] is True for score in scores),
        "no_overlap_not_observable": sum(score["authority_separation"]["no_allowed_held_overlap"] is None for score in scores),
        "scope_expansion_correct": sum(score["scope_expansion"]["correct"] is True for score in scores),
        "scope_expansion_not_observable": sum(score["scope_expansion"]["correct"] is None for score in scores),
        "scope_expansion_type_correct": sum(score["scope_expansion"]["type_correct"] is True and score["scope_expansion"]["correct"] is True for score in scores),
        "review_status_exact": sum(score["review_status"]["exact_match"] is True for score in scores),
...
def paired(stage: dict[str, Any], interface: dict[str, Any]) -> dict[str, Any]:
    ...
    for task_id in TASK_IDS:
        ...
        delta = compare_components(
            component_vector(before, parse_valid=stage[task_id]["raw_parse_valid"]),
            component_vector(after, parse_valid=interface[task_id]["raw_parse_valid"]),
        )
        result[task_id] = {
            **delta,
            "stage_a_semantic_fields_correct": before["semantic_fields_correct"],
            "interface_semantic_fields_correct": after["semantic_fields_correct"],
            "stage_a_field_types_all_correct": all(before["field_types_correct"].values()),
            "interface_field_types_all_correct": all(after["field_types_correct"].values()),
        }
    return result
