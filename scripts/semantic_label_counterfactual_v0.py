...
ROOT = runtime.ROOT
RUN = ROOT / ".work/model_size_supplier_floor/semantic_label_counterfactual_v0/run_20260823T220000Z"
...
def specs() -> list[tuple[str, str, str]]:
    return [
        ...
        ("slcf-002", "inspect", "Can you tell me whether docs/research/Q2_WILLOW_2026-08-23.json is currently stored here?"),
        ...
        ("slcf-005", "observe_presence", "Study docs/research/Q5_ALMOND_2026-08-23.json for a moment."),
...
def prepare(out: Path) -> None:
    ...
    manifest = {"schema": "zth_semantic_label_counterfactual_v0_manifest", "status": "prepared_model_free", "task_count": 6, "paired_opportunities": 6, "planned_model_calls": 12, "model_calls_made": 0, "tool_calls_made": 0, "response_files": 0, "true_fallback_eligibility": 6, "historical_request_reuse": 0, "paired_input_identity_audit": True, "authority_provenance_audit": True, "definition_equivalence_audit": True, "semantic_enum_positions_identical": True, "counterbalance_audit": True, "a_first_count": 3, "b_first_count": 3, "a_second_count": 3, "b_second_count": 3, "runtime_evaluator_influence": 0, "model_output_granted_authority": 0, "qualification_change": False, "teacher_calls": 0, "30b_calls": 0, "external_calls": 0, "retries": 0, "driver_sha256": sha_file(Path(__file__).resolve()), "model_settings": model_settings(), "arm_a_labels": ARM_A, "arm_b_labels": ARM_B, "arm_a_enum": ARM_ENUMS["A"], "arm_b_enum": ARM_ENUMS["B"]}
...
def execute(out: Path) -> None:
    ...
    for item in read_json(out / "execution_order.json")["order"]:
        ...
        candidate, parse_valid, contract_valid, diagnostics = parse_response(response.content, item["arm"]); calls += 1; admissible = candidate in ("observe_presence", "inspect") if candidate else False
        write_json(task_dir / "candidate_validation.json", {"candidate": candidate, "parse_valid": parse_valid, "contract_valid": contract_valid, "candidate_valid": candidate in ARM_ENUMS[item["arm"]] if candidate else False, "candidate_admissible": admissible, "diagnostics": diagnostics})
        if contract_valid and admissible:
            canonical_operation = ARM_CANONICAL[item["arm"]][candidate]
