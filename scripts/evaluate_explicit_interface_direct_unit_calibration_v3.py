#!/usr/bin/env python3
"""Pure V3 evaluator with transport-aware protocol validity.

This module is scoring-only and must be loaded only after raw acquisition is
sealed. It has no filesystem, network, model, or supplier dependency.
"""
...
def _declared_semantic_text(parsed: dict[str, Any]) -> str:
    values: list[str] = []
    for field in ("known_facts", "uncertainty", "next_step"):
...
def evaluate(
    ...
    prohibited_actions_not_observed = protocol_metadata.get("prohibited_actions_not_observed") is True
    required_fields = list(contract["required_fields"])
    required_fields_valid = parse_valid and isinstance(parsed, dict) and all(field in parsed for field in required_fields)
    ...
    if required_field_types_valid:
        for field, type_name in contract["field_types"].items():
            ...
            if type_name == "string" and not isinstance(value, str):
                ...
                required_field_types_valid = False
    parse_dimension = parse_valid and isinstance(parsed, dict) and not duplicate_keys
    ...
    dimensions = {
        ...
        "PROHIBITED_ACTIONS_NOT_OBSERVED": prohibited_actions_not_observed,
