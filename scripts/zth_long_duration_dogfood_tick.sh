...
EOF
  ...
  printf '%s\n%s\n%s\n' \
    "tests_or_fixtures" \
    "Review long-duration dogfood closeout before selecting next lane." \
    $'Review the long-duration dogfood closeout evidence and choose the next bounded lane before adding more implementation. Do not add queue writing, queue insertion, queue running, automatic handoff, router automation, repo mutation, fixture import, training capture, promotion, deployment, or downstream-use authority.'
}
