...
def closeout(out: Path) -> None:
    ...
    for td in sorted((out / "tasks").glob("*")):
        ...
        plan0 = json.loads((td / "capability_plan_0.json").read_text())
        actual0 = list(plan0["derived_required_capabilities"])
        expected_all = evaluator.get("expected_required_capabilities", [])
        expected0 = [TOOL_CAPABILITY] if TOOL_CAPABILITY in expected_all else list(expected_all)
