...
- ZTH Task Front Door V1 is closed: per-question Historian outcomes (bound, insufficient, or failed, with contract-valid zero-citation answers preserved as separate non-bound artifacts and true failures blocking) landed through the reviewed repair session, the first real dogfood task completed end to end through the front door in one pass and removed the pre-existing repo-health boundary-language finding, and human review of both the repair and the dogfood is recorded as accepted; future work remains in a supported in-front-door coding-agent execution path, multi-step tasks, and dependency-aware scope.
