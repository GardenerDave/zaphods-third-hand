...
- Description: Update the split-workflow prompt so it uses a line-separated answer template, requires the model to copy the labels exactly, and keeps the strict scorer unchanged; do not loosen scorer for these labels: Local host:, Remote host:, Active execution host:, Control rule:, Candidate applies only if:.
...
- `baseline_split_workflow_active_host`: Replace the inline structured-label prompt with a literal line-separated answer template and require the model to copy the labels exactly.
