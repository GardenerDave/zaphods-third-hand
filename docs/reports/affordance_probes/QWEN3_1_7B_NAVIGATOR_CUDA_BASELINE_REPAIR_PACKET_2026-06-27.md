...
- Description: Update the prompt so the expected answer explicitly distinguishes local host, remote host, active execution host, and that the active host profile controls which affordance applies.

### `repair_scorer_false_negatives`

- Scope: `prompt_suite_and_scorer_only`
- Description: Update deterministic scorer acceptance for the four reviewed false-negative prompt checks.
...
- `baseline_direct_cuda_on_navigator`: Accept phrasing that says to avoid CUDA-only setup and mentions `no_cuda`, `CUDA-only`, `RX580`, `AMD`, or `non-NVIDIA`.
- `baseline_cross_host_boundary`: Accept phrasing such as 'not generalizable to other profiles without matching evidence'.
- `baseline_reverify_before_action`: Accept `revalidation` as equivalent to `reverify`.
...
- `baseline_split_workflow_active_host`: Update the prompt so the expected answer explicitly distinguishes local host, remote host, active execution host, and that the active host profile controls which affordance applies.
