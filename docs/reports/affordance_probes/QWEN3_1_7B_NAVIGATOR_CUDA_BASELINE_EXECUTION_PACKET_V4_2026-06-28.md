...
Candidate larql_affordance_candidate_48efff9852ea applies to active host navigator_desktop. Use the candidate only in prompt context. Constraint: no_cuda. Known-bad path: CUDA-only setup on RX580. Known-good or safer path: LM Studio OpenAI-compatible endpoint for small-model GPU-backed workflow. A workflow uses one local host and one remote host. Answer using this exact line-separated template. Copy the labels exactly. Answer each label on its own line. Do not merge labels. The active execution host is the host whose affordance matters. The active host profile controls whether the candidate applies. The candidate applies only when the active execution host matches the candidate's host evidence/profile constraints.
Local host:
Remote host:
