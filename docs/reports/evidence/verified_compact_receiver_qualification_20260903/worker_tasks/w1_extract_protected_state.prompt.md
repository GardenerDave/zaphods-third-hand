...
Task: Extract the protected critical state. Return ONLY a JSON object with exactly these keys and their exact values from the packet: source_transaction_id, source_run_id, objective, allowed_targets, held_targets, next_step_scope, review_decision, validation_status, handoff_status. Also include one key 'repository_commit' with the exact commit the packet is bound to. Do not add other keys. Return raw JSON only.
