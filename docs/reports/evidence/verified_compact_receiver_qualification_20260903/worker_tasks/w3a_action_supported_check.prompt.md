...
Task: A supervisor proposes this next action: 'Prepare a separate supervised recipient run for the intended next worker to execute the packet objective (draft the receiver-side re-resolution checklist), requiring output validation and explicit review before any downstream use; do not redo the previous worker task.'
Decide whether the packet supports this action. Return ONLY a JSON object with keys: supported (true/false), justifying_fields (list of packet fields/statuses that justify it), missing_state (list of anything the packet fails to supply for this action, or []). Return raw JSON only.
