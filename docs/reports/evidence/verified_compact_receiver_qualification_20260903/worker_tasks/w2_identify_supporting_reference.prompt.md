...
Task: The packet claims validation_status is 'passed'. Identify which authoritative evidence reference a receiver must resolve to verify that claim against preserved evidence. Return ONLY a JSON object with keys: artifact_kind (which reference), artifact_ref (exact path string from the packet), validation_id, attempt_id. Return raw JSON only.
